...
| **DNS 修复** | 有 root：`dns53.js` 本地 DNS 转发器 + `.bashrc` 常驻（**自动优先手机当前 DNS，IPv4/IPv6 独立探测、缺失时公共 DNS 兜底；AAAA 默认屏蔽防境外 IPv6 卡死，境内 IPv6 可用时设 `DNS53_DISABLE_AAAA=0` 放开**）；无 root：`dns-bootstrap.js` 实测校验候选 DNS 后写入 `resolv.conf` + proot 绑定 + `.bashrc` 常驻（每 60s 重测）。**Android 没有 `/etc/resolv.conf`**，musl 解析器回退 `127.0.0.1:53`（无人监听）→ 每次请求卡满 5s 超时，报 `error sending request`。两套方案都会校验应答质量并跳过被过滤的上游 |

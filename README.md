...
### 0.157.1 也没修好（验证时注意 TTY）

**0.157.1 依然存在该问题。** 曾一度以为它修好了，是验证方法出错：用 `codex ... < /dev/null`
跑时，没有 TTY，codex 在 "stdin is not a terminal" 这一步就退出了，**根本没走到 daemon 逻辑**，
看起来"正常"其实是被提前拦下。必须在**真 pty** 下测才能复现：

```bash
# 真 TTY 复现 (需要 python3)
python3 -c "
import pty,os,select,signal,sys,time
pid,fd=pty.fork()
if pid==0: os.execvp('codex',['codex'])
out=b''; t0=time.time()
...
- daemon 包一直锁在 **0.157.0**（`~/.codex/packages/app-server-daemon/auto-update-version` 与 `current` 软链）
- `codex app-server daemon update` 会去拉 `https://chatgpt.com/codex/install.sh`，该域名在国内网络下超时
  ——这就是为什么它永远停在 0.157.0，也意味着短期内别指望靠升级 daemon 解决
- 因此本脚本的规避是**长期必要的**，不是临时补丁
